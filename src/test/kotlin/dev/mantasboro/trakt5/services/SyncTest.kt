...
class SyncTest : BaseTestCase() {
    ...
    fun test_getPlayback() {
        ...
        val episodeDelegate = GenericProgressData(episode = episode, progress = 25.0)
        val episodeProgress =
            ScrobbleProgress(data = episodeDelegate, app_date = APP_DATE, app_version = APP_VERSION)
        ...
        val movie = SyncMovie().id(MovieIds(data = BaseIdsData(tmdb = 157336)))
        val movieDelegate = GenericProgressData(movie = movie, progress = 32.0)
        val movieProgress =
            ScrobbleProgress(data = movieDelegate, app_date = APP_DATE, app_version = APP_VERSION)
    ...
    private fun buildMovieIds(): MovieIds {
        return MovieIds(BaseIdsData(tmdb = TestData.MOVIE_TMDB_ID))
    }
